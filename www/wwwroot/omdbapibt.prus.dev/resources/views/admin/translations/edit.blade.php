...
@section('content')
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <h2 class="text-xl font-semibold">{{ __('admin.edit_translation') }}</h2>

        <form method="POST" action="{{ route('admin.translations.update', $translation) }}" class="mt-4 space-y-6">
            @csrf
            @method('PUT')
                ...
                @foreach ($locales as $locale)
                    <div>
                        ...
                        <textarea
                            ...
                            class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring"
                        >{{ old("text.$locale", $translation->getTranslation('text', $locale, false)) }}</textarea>
                        @error("text.$locale")
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
