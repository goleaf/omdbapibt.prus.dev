...
<body class="min-h-screen bg-transparent text-slate-100">
    <div class="relative isolate flex min-h-screen flex-col">
        ...
        <header class="border-b border-slate-800/50 bg-slate-900/65 backdrop-blur">
            <div class="mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-5">
                <a href="{{ route('home') }}" class="flex items-center gap-2 text-lg font-semibold tracking-wide text-slate-100">
                    <span class="text-emerald-400">◎</span>
                    <span>{{ __('ui.nav.brand.primary') }}<span class="text-emerald-400">{{ __('ui.nav.brand.secondary') }}</span></span>
                </a>

                <nav class="hidden items-center gap-8 text-sm font-medium md:flex">
                    <a href="{{ route('home') }}" class="transition hover:text-emerald-300">{{ __('ui.nav.links.home') }}</a>
                    <a href="{{ route('browse') }}" class="transition hover:text-emerald-300">{{ __('ui.nav.links.browse') }}</a>
                    <a href="{{ route('pricing') }}" class="transition hover:text-emerald-300">{{ __('ui.nav.links.pricing') }}</a>
                    <a href="{{ route('ui.components') }}" class="transition hover:text-emerald-300">{{ __('ui.nav.links.components') }}</a>
                    @auth
                        <a href="{{ route('account') }}" class="transition hover:text-emerald-300">{{ __('ui.nav.links.account') }}</a>
                        @if (auth()->user()?->isAdmin())
                            <a href="{{ route('admin.analytics') }}" class="transition hover:text-emerald-300">{{ __('ui.nav.links.admin') }}</a>
                        @endif
                    @endauth
                </nav>
                ...
                <div class="flex items-center gap-3 text-sm">
                    ...
                    @auth
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="rounded-full border border-slate-700 px-4 py-1.5 text-slate-200 transition hover:border-emerald-400 hover:text-emerald-200">{{ __('ui.nav.auth.logout') }}</button>
                        </form>
                    @else
                        @if (Route::has('login'))
                            <a href="{{ route('login') }}" class="rounded-full border border-slate-700 px-4 py-1.5 transition hover:border-emerald-400 hover:text-emerald-200">{{ __('ui.nav.auth.login') }}</a>
                        @endif
        ...
        <footer class="border-t border-slate-800/40 bg-slate-900/70 py-8">
            <div class="mx-auto flex w-full max-w-7xl flex-col gap-4 px-6 text-sm text-slate-400 sm:flex-row sm:items-center sm:justify-between">
