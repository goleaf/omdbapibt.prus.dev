<div class="grid gap-6 lg:grid-cols-[minmax(0,420px)_1fr]">
    <div class="space-y-4 rounded-3xl border border-slate-800/60 bg-slate-900/70 p-6">
        <div class="flex items-start justify-between">
            <div>
                <h2 class="text-lg font-semibold text-white">{{ __('ui.admin.panel.sections.movies.title') }}</h2>
                <p class="mt-1 text-sm text-slate-400">{{ __('ui.admin.panel.sections.movies.subtitle') }}</p>
            </div>
            <button
                type="button"
                wire:click="create"
        ...
        <form wire:submit.prevent="save" class="space-y-4">
            <div>
                <label class="text-xs uppercase tracking-[0.35em] text-slate-400" for="movie-title">{{ __('ui.admin.panel.fields.title') }}</label>
                <input
                    id="movie-title"
                    type="text"
                    ...
                    placeholder="{{ __('ui.admin.panel.placeholders.movie_title') }}"
                />
                @error('form.title')
            ...
            <div>
                <label class="text-xs uppercase tracking-[0.35em] text-slate-400" for="movie-slug">{{ __('ui.admin.panel.fields.slug') }}</label>
                <input
                    id="movie-slug"
                    type="text"
            ...
            <div>
                <label class="text-xs uppercase tracking-[0.35em] text-slate-400" for="movie-status">{{ __('ui.admin.panel.fields.status') }}</label>
                <input
                    id="movie-status"
                    type="text"
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="text-xs uppercase tracking-[0.35em] text-slate-400" for="movie-release">{{ __('ui.admin.panel.fields.release_date') }}</label>
                    <input
                        id="movie-release"
                        type="date"
                        wire:model.defer="form.release_date"
                        class="mt-1 w-full rounded-2xl border border-slate-800 bg-slate-950/70 px-4 py-2 text-sm text-slate-100 focus:border-emerald-400 focus:outline-none"
                    />
                ...
                <div>
                    <label class="text-xs uppercase tracking-[0.35em] text-slate-400" for="movie-vote">{{ __('ui.admin.panel.fields.vote_average') }}</label>
                    <input
                        id="movie-vote"
                        type="number"
                        step="0.1"
            ...
            <div class="flex items-center gap-3">
                ...
                />
                <label for="movie-adult" class="text-sm text-slate-300">{{ __('ui.admin.panel.fields.adult') }}</label>
            </div>
            @error('form.adult')
                <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
            @enderror
        ...
        <div class="rounded-3xl border border-slate-800/60 bg-slate-900/70 p-4">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div class="w-full sm:max-w-xs">
                    <label class="text-xs uppercase tracking-[0.35em] text-slate-400" for="movie-search">{{ __('ui.admin.panel.fields.search') }}</label>
                    <input
                        id="movie-search"
                        type="search"
                        wire:model.live="search"
                        placeholder="{{ __('ui.admin.panel.placeholders.search_movies') }}"
        ...
        <div class="overflow-hidden rounded-3xl border border-slate-800/60 bg-slate-900/70">
            <table class="min-w-full divide-y divide-slate-800 text-sm">
                <thead class="bg-slate-900/80 text-slate-400">
                    <tr>
                        <th class="px-6 py-3 text-left">{{ __('ui.admin.panel.table.movie') }}</th>
                        <th class="px-6 py-3 text-left">{{ __('ui.admin.panel.fields.status') }}</th>
                        <th class="px-6 py-3 text-left">{{ __('ui.admin.panel.fields.release_date') }}</th>
                ...
                <tbody class="divide-y divide-slate-800 text-slate-200">
                    @forelse ($this->records as $movie)
                        <tr class="hover:bg-slate-900/60" wire:key="movie-{{ $movie->id }}">
                            <td class="px-6 py-4">
                                <div class="font-semibold text-white">{{ $movie->localizedTitle('en') }}</div>
                                <div class="text-xs text-slate-400">{{ $movie->slug }}</div>
                            </td>
                            <td class="px-6 py-4">{{ $movie->status ?? '—' }}</td>
                            <td class="px-6 py-4">{{ optional($movie->release_date)->toDateString() ?? '—' }}</td>
                            <td class="px-6 py-4">{{ $movie->vote_average !== null ? number_format($movie->vote_average, 1) : '—' }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <button
                                        type="button"
                                        wire:click="edit({{ $movie->id }})"
