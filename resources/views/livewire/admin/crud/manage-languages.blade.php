<div class="grid gap-6 lg:grid-cols-[minmax(0,360px)_1fr]">
    <div class="space-y-4 rounded-3xl border border-slate-800/60 bg-slate-900/70 p-6">
        <div class="flex items-start justify-between">
            <div>
                <h2 class="text-lg font-semibold text-white">{{ __('ui.admin.panel.sections.languages.title') }}</h2>
                <p class="mt-1 text-sm text-slate-400">{{ __('ui.admin.panel.sections.languages.subtitle') }}</p>
            </div>
            <button
                type="button"
                wire:click="create"
        ...
        <form wire:submit.prevent="save" class="space-y-4">
            <div>
                <label class="text-xs uppercase tracking-[0.35em] text-slate-400" for="language-name">{{ __('ui.admin.panel.fields.name') }}</label>
                <input
                    id="language-name"
                    type="text"
                    ...
                    placeholder="{{ __('ui.admin.panel.placeholders.language_name') }}"
                />
                @error('form.name')
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="text-xs uppercase tracking-[0.35em] text-slate-400" for="language-code">{{ __('ui.admin.panel.fields.code') }}</label>
                    <input
                        id="language-code"
                        type="text"
                ...
                <div>
                    <label class="text-xs uppercase tracking-[0.35em] text-slate-400" for="language-native-name">{{ __('ui.admin.panel.fields.native_name') }}</label>
                    <input
                        id="language-native-name"
                        type="text"
                        wire:model.defer="form.native_name"
                        class="mt-1 w-full rounded-2xl border border-slate-800 bg-slate-950/70 px-4 py-2 text-sm text-slate-100 focus:border-emerald-400 focus:outline-none"
                        placeholder="{{ __('ui.admin.panel.placeholders.native_name') }}"
                    />
                    @error('form.native_name')
                        <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                />
                <label for="language-active" class="text-sm text-slate-300">{{ __('ui.admin.panel.fields.active') }}</label>
            </div>
            @error('form.active')
                <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
            @enderror
        ...
        <div class="rounded-3xl border border-slate-800/60 bg-slate-900/70 p-4">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div class="w-full sm:max-w-xs">
                    <label class="text-xs uppercase tracking-[0.35em] text-slate-400" for="language-search">{{ __('ui.admin.panel.fields.search') }}</label>
                    <input
                        id="language-search"
                        type="search"
        ...
        <div class="overflow-hidden rounded-3xl border border-slate-800/60 bg-slate-900/70">
            <table class="min-w-full divide-y divide-slate-800 text-sm">
                <thead class="bg-slate-900/80 text-slate-400">
                    <tr>
                        <th class="px-6 py-3 text-left">{{ __('ui.admin.panel.table.language') }}</th>
                        <th class="px-6 py-3 text-left">{{ __('ui.admin.panel.fields.code') }}</th>
                        <th class="px-6 py-3 text-left">{{ __('ui.admin.panel.fields.native_name') }}</th>
                ...
                <tbody class="divide-y divide-slate-800 text-slate-200">
                    @forelse ($this->records as $language)
                        <tr class="hover:bg-slate-900/60" wire:key="language-{{ $language->id }}">
                            <td class="px-6 py-4 font-semibold text-white">{{ $language->localizedName('en') }}</td>
                            <td class="px-6 py-4 uppercase">{{ $language->code }}</td>
                            <td class="px-6 py-4">{{ $language->localizedNativeName('en') }}</td>
                            <td class="px-6 py-4">{{ $language->active ? __('ui.admin.panel.labels.active') : __('ui.admin.panel.labels.inactive') }}</td>
                            <td class="px-6 py-4">
